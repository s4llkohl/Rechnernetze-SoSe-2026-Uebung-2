    ...
    private static void loop(DatagramSocket socket, String ip, int port, boolean first){
        ...
        while (true) {
            try {
                ...
                // neuer Knoten wird hier eingefügt, falls anderer nicht erreichbar ist
                public Token insertAfter(Endpoint target, Endpoint newEndpoint) {
                    LinkedList<Endpoint> temp = new LinkedList<>();
                    boolean inserted = false;

                    while (!ring.isEmpty()) {
                        Endpoint current = ring.poll();
                        temp.offer(current);

                        if (!inserted && current.equals(target)) {
                            temp.offer(newEndpoint);
                            inserted = true;
                        }
                    }
                ...
                if (!sent) {
                    System.out.printf("Token %s:%d removed!%n", next.ip(), next.port());
                    rc.removeEndpoint(next);

                    Token.Endpoint newNode = new Token.Endpoint(ip, port);
                    rc.insertAfter(next, newNode);
