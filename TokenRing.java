            ...
            try {
                ...
                for (int i = 0; i < maxRetries && !sent; i++) {
                    try {
                        ...
                        System.out.printf(
                                "Failed to send to %s:%d - Try: %d/%d%n",
                ...
                if (!sent) {
                    System.out.printf("Endpoint %s:%d removed from ring.%n", next.ip(), next.port());
                    rc.removeEndpoint(next);

                    if (rc.length() == 0) {
                        System.out.println("No more endpoints in ring.");
                        break;
                    }
                    ...
                    try {
                        ...
                        System.out.println("Could not send token to new next endpoint: " + e.getMessage());
                    }
                }
